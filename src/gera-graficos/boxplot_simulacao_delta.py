...
def preparar_dados(df):
    ...
    # Converter coluna 'Ajuste.Delta' para numérico
    df['Ajuste.Delta'] = pd.to_numeric(df['Ajuste.Delta'], errors='coerce')
    
    # Converter coluna '# Pregões Vol.' para numérico
    df['# Pregões Vol.'] = pd.to_numeric(df['# Pregões Vol.'], errors='coerce')
    
    # Remover linhas com valores nulos
    df_clean = df.dropna(subset=['Saldo Final', 'Ajuste.Delta', '# Pregões Vol.', 'Simulação'])
    
    print(f"Dados após limpeza: {df_clean.shape}")
    print(f"Valores únicos em 'Simulação': {df_clean['Simulação'].unique()}")
    print(f"Valores únicos em 'Ajuste.Delta': {sorted(df_clean['Ajuste.Delta'].unique())}")
...
def gerar_boxplot_simulacao(df, output_dir):
    ...
    plt.title('Distribuição do Saldo Final por Estado do Delta\n(Delta Hedge - Ajuste por Delta)', 
              fontsize=14, fontweight='bold', pad=20)
    plt.xlabel('Estado do Delta (Inicial → Final)', fontsize=12)
    ...
    # Salvar gráfico
    output_path = output_dir / 'boxplot_simulacao_delta.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Boxplot por Simulação salvo em: {output_path}")
    
    plt.show()

def gerar_boxplot_ajuste_delta(df, output_dir):
    """Gera boxplot por 'Ajuste.Delta'."""
    plt.figure(figsize=(14, 8))
    
    # Criar boxplot sem preenchimento preto
    sns.boxplot(data=df, x='Ajuste.Delta', y='Saldo Final', 
                palette='Set3', linewidth=1.5)
    
    plt.title('Distribuição do Saldo Final por Limite de Delta\n(Delta Hedge - Ajuste por Delta)', 
              fontsize=14, fontweight='bold', pad=20)
    plt.xlabel('Limite de Delta', fontsize=12)
    plt.ylabel('Saldo Final (R$)', fontsize=12)
    ...
    # Salvar gráfico
    output_path = output_dir / 'boxplot_ajuste_delta.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Boxplot por Ajuste Delta salvo em: {output_path}")
...
def gerar_boxplot_pregões_vol(df, output_dir):
    ...
    # Salvar gráfico
    output_path = output_dir / 'boxplot_pregões_vol_delta.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def gerar_boxplot_combinado(df, output_dir):
    """Gera boxplot combinando Ajuste Delta e Pregões Vol."""
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Análise Combinada: Limite Delta vs Período de Volatilidade\n(Delta Hedge - Ajuste por Delta)', 
                 fontsize=16, fontweight='bold')
    
    # Subplot 1: Boxplot por Ajuste Delta
    sns.boxplot(data=df, x='Ajuste.Delta', y='Saldo Final', 
                palette='Set2', linewidth=1.5, ax=axes[0,0])
    axes[0,0].set_title('Saldo Final por Limite de Delta')
    axes[0,0].set_xlabel('Limite de Delta')
    ...
    axes[1,0].set_title('Saldo Final por Estado do Delta')
    axes[1,0].set_xlabel('Estado do Delta (Inicial → Final)')
    axes[1,0].set_ylabel('Saldo Final (R$)')
    axes[1,0].tick_params(axis='x', rotation=45)
    axes[1,0].grid(True, alpha=0.3)
    
    # Subplot 4: Violin plot combinado
    sns.violinplot(data=df, x='Ajuste.Delta', y='Saldo Final', 
                   hue='# Pregões Vol.', palette='Set2', ax=axes[1,1])
    axes[1,1].set_title('Distribuição por Limite Delta e Volatilidade')
    axes[1,1].set_xlabel('Limite de Delta')
    ...
    # Salvar gráfico
    output_path = output_dir / 'boxplot_combinado_delta.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"Boxplot combinado salvo em: {output_path}")
    
    plt.show()

def gerar_boxplot_categorias_detalhado(df, output_dir):
    ...
    # Subplot 4: Violin plot para estado inicial ITM (Limite Delta e Volatilidade)
    df_itm = df[df['Estado_Inicial_Delta'] == 'ITM'].copy()
    if not df_itm.empty:
        sns.violinplot(data=df_itm, x='Ajuste.Delta', y='Saldo Final', 
                       hue='# Pregões Vol.', palette='Set2', ax=axes[1,1])
        axes[1,1].set_title('Distribuição Inicial ITM: Limite Delta e Volatilidade')
        axes[1,1].set_xlabel('Limite de Delta')
        axes[1,1].set_ylabel('Saldo Final (R$)')
        axes[1,1].grid(True, alpha=0.3)
    else:
    ...
    # Salvar gráfico
    output_path = output_dir / 'boxplot_estados_delta_detalhado.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def gerar_heatmap_correlacao(df, output_dir):
    ...
    numeric_cols = ['Saldo Final', 'Ajuste.Delta', '# Pregões Vol.', '# Ajustes']
    ...
    # Salvar gráfico
    output_path = output_dir / 'heatmap_correlacao_delta.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
...
def gerar_estatisticas_descritivas(df, output_dir):
    ...
    stats_file = output_dir / 'estatisticas_descritivas_delta.txt'
    
    with open(stats_file, 'w', encoding='utf-8') as f:
        f.write("ESTATÍSTICAS DESCRITIVAS - DELTA HEDGE AJUSTE POR DELTA\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write("\n\nPOR LIMITE DE DELTA:\n")
        f.write("-" * 40 + "\n")
        for limite in sorted(df['Ajuste.Delta'].unique()):
            subset = df[df['Ajuste.Delta'] == limite]
            f.write(f"\nLimite {limite}:\n")
            f.write(f"  Observações: {len(subset)}\n")
            f.write(f"  Média: R$ {subset['Saldo Final'].mean():.2f}\n")
            f.write(f"  Mediana: R$ {subset['Saldo Final'].median():.2f}\n")
...
def main():
    """Função principal."""
    # Configurar caminhos
    arquivo_excel = Path('dados/SimulacaoPeloDelta.xlsx')
    ...
    print("=" * 60)
    print("GERADOR DE BOXPLOTS - DELTA HEDGE AJUSTE POR DELTA")
    print("=" * 60)
    ...
    # Boxplot de categorias detalhado
    gerar_boxplot_categorias_detalhado(df_clean, output_dir)
    ...
    print("- boxplot_estados_delta_detalhado.png")
    print("- heatmap_correlacao_delta.png")
    print("- estatisticas_descritivas_delta.txt")
