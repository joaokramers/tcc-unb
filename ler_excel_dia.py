...
# Lê o arquivo Excel
df = pd.read_excel('dados/SimulacaoPeloDia.xlsx')
...
# Gera tabela LaTeX
print("\n" + "="*50)
print("TABELA LATEX - AJUSTE PELO DIA:")
print("="*50)
...
latex_table += "\\small\n"
latex_table += "\\caption{Resultados da Simulação de Delta Hedge - Ajuste por Frequência Temporal}\n"
latex_table += "\\label{tab:simulacao-dia}\n"
...
latex_table += "& \\textbf{(R\\$)} & & \\textbf{Ini→Fin} & \\textbf{Freq/VOL} & \\textbf{(\\#)} & \\textbf{(R\\$)} \\\\\n"
latex_table += "\\hline\n"
...
for i, row in df.iterrows():
    ...
    # Formata parâmetros
    params = f"{freq_ajuste:.0f}/{pregoes_vol:.0f}" if pd.notna(freq_ajuste) and pd.notna(pregoes_vol) else "-"
